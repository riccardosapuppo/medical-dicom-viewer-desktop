...
function generate(target: string): void {
  ...
  process.stdout.write(
    `${files} images, ${(bytes / (1024 * 1024)).toFixed(1)} MB, in ${path.relative(process.cwd(), target) || target}\n`
  );
}
...
if (fs.existsSync(target)) {
  // Regenerating on top of an old folder leaves whatever was there before, and
  // the indexer would report both. Say it rather than silently mixing them.
  process.stderr.write(`${target} already exists. Remove it first, or pass another path.\n`);
  process.exit(1);
}
